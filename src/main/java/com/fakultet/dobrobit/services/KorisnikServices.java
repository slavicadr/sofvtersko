...
public class KorisnikServices {
    ...
    public Korisnik registrujKorisnika(Korisnik korisnik) {
        ...
        // Validacija specifična za volontera (SRS 3.4.3 — mora imati bar jednu uslugu)
        // Napomena: usluge se šalju odvojeno kroz UslugaProizvod endpoint,
        // ali volonter mora biti verifikovan od admina prije nego postane aktivan (SRS 3.4.2)
        if (TipKorisnika.volonter.equals(korisnik.getTipKorisnika())) {
            if (korisnik.getOpis() == null || korisnik.getOpis().isBlank()) {
                throw new RuntimeException("Opis je obavezan za volontere!");
            }
        }
