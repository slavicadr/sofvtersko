...
public class UslugaProizvodController {
    ...
    @PostMapping
    public ResponseEntity<UslugaProizvod> kreirajNovu(@RequestBody UslugaProizvod usluga) {
        // Servis će automatski postaviti datum i status "na_cekanju"
    ...
    public ResponseEntity<String> obrisi(@PathVariable int id) {
        try {
            service.obrisi(id);
            return ResponseEntity.ok("Usluga/Proizvod uspešno obrisan.");
        } catch (Exception e) {
            return ResponseEntity.notFound().build();
        }
    }
