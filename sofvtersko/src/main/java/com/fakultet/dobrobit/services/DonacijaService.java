...
public class DonacijaService {
    ...
    public Donacija kreirajDonaciju(Integer donatorId, int pomocId, BigDecimal iznos, String nacinPlacanja, boolean anonimno) {
        Korisnik donator = null;
        if (donatorId != null && donatorId > 0) {
            donator = korisnikRepository.findById(donatorId)
        ...
        donacija.setAnonimno(anonimno);
        donacija.setDatumDonacije(LocalDateTime.now());
        donacija.setStatusDonacije("placeno");
        donacija.setStatusPlacanja("placeno");
