...
public class OcjenaRecenzijaController {
    ...
    // Dodaj recenziju — samo kupac ili volonter koji je kupio uslugu (SRS UC-6)
    // Body: { "kupovinaId": 1, "brojZvjezdica": 5, "komentar": "Odlično!" }
    @PostMapping("/dodaj")
    public ResponseEntity<?> dodaj(@RequestBody Map<String, Object> podaci,
        ...
        try {
            ...
            OcjenaRecenzija nova = service.dodajRecenziju(kupovinaId, principal.getKorisnikId(), brojZvjezdica, komentar);
    ...
    // Volonter odgovara na recenziju
    @PatchMapping("/{id}/odgovor")
    public ResponseEntity<?> odgovori(@PathVariable int id,
                                      @RequestBody Map<String, String> body,
                                      @AuthenticationPrincipal Korisnik principal) {
        if (principal == null) return ResponseEntity.status(401).body("Nije prijavljen.");
        try {
            String odgovor = body.get("odgovor");
            if (odgovor == null || odgovor.isBlank()) return ResponseEntity.badRequest().body("Odgovor ne smije biti prazan.");
            OcjenaRecenzija updated = service.dodajOdgovor(id, principal.getKorisnikId(), odgovor);
